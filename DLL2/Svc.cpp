...
{ 
    ...
    ServiceTable[0].lpServiceName = NULL;
    ServiceTable[0].lpServiceProc = NULL;
...
//   None.
//
VOID WINAPI SvcMain( DWORD dwArgc, wchar_t** argv )
{
    // Register the handler function for the service

    gSvcStatusHandle = RegisterServiceCtrlHandlerExW(serviceName, (LPHANDLER_FUNCTION_EX)SvcCtrlHandler, argv[0]);

    if (gSvcStatusHandle == (SERVICE_STATUS_HANDLE)0)
    { 
        return;
    }
...
VOID SvcInit( DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    {
        std::cout << L"WTSEnumerateSessionsW returns TRUE, sessionsCount = " << sessionsCount << std::endl;
        for (DWORD i = 0; i < sessionsCount; ++i)
        {

            if (wtsSessions[i].SessionId != 0)
            {
                HANDLE userToken;
                if (WTSQueryUserToken(wtsSessions[i].SessionId, &userToken))
                {

                    WCHAR commandLine[] = L"\"%SystemRoot%\\system32\\WindowsPowerShell\\v1.0\\powershell.exe\" notepad example ";

                    std::cout << L"Starting " << commandLine << std::endl;
                    PROCESS_INFORMATION pi{};
                    STARTUPINFO si = { sizeof(STARTUPINFO) };
                    if (CreateProcessAsUserW(userToken, NULL, commandLine, NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi))
                    {
                        std::cout << L"Process started pid = " << pi.dwProcessId << std::endl;
                    ...
                    {
                        std::cout << L"Failed to start process. Error code = " << GetLastError() << std::endl;
                    }
                    CloseHandle(userToken);
                }
                else
                {
                    std::cout << L"WTSQueryUserToken failed. Error code = " << GetLastError() << std::endl;
                }
            }
        }
    }
    ...
    {
        std::cout << L"WTSEnumerateSessionsW returns FALSE, error code = " << GetLastError() << std::endl;
    }
