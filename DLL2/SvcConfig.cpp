...
TCHAR szCommandConfig[10];
TCHAR szSvcNameConfig[80];

VOID __stdcall DisplayUsageConfig(void);
...
int __cdecl _tmain(int argc, TCHAR* argv[])
{
    ...
    StringCchCopy(szCommandConfig, 10, argv[1]);
    StringCchCopy(szSvcNameConfig, 80, argv[2]);

    if (lstrcmpi(szCommandConfig, TEXT("query")) == 0)
        DoQuerySvc();
    else if (lstrcmpi(szCommandConfig, TEXT("describe")) == 0)
        DoUpdateSvcDesc();
    else if (lstrcmpi(szCommandConfig, TEXT("disable")) == 0)
        DoDisableSvc();
    else if (lstrcmpi(szCommandConfig, TEXT("enable")) == 0)
        DoEnableSvc();
    else if (lstrcmpi(szCommandConfig, TEXT("delete")) == 0)
        DoDeleteSvc();
    else
    {
...
VOID __stdcall DisplayUsageConfig()
{
    printf("Description:\n");
...
VOID __stdcall DoQuerySvc()
{
    ...
    schService = OpenService(
        ...
        szSvcNameConfig,             // name of service 
...
VOID __stdcall DoUpdateSvcDesc()
{
    ...
    sd.lpDescription = (LPTSTR)TEXT("This is a test description");
